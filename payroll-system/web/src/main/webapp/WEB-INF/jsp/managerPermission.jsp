		...
		<c:forEach items="${roleList}" var="role" varStatus="sno">
			<tr>
				...
				<td><button type="button"
						onclick="getPermission('${pageContext.request.contextPath}/company/getPermissions', '${role.id}','${role.roleName}')">Edit
						Permissions</button></td>
			</tr>
		</c:forEach>
	</table>
	<br />
	<c:url value="/company/managePermissions" var="actionUrl" />
						...
						<c:if test="${subMenu eq sectionModel.section.subMenu}">
							<tr>
								...
								<td><form:checkboxes items="${permissionTypeList}"
										...
										itemLabel="name"
										onclick="processRows('${row.index}', this.value)" /><input type="hidden" id="lastRowNo"
									value="${row.index}" /></td>
							</tr>
						</c:if>
					</c:forEach>
