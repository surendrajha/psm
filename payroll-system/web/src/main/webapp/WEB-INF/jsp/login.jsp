...
<div class="signupsection">
	<!--<c:if test="${not empty error}">
		<div class="errorblock" style="color: red">
			...
			<c:set var="errorString"
				value="${sessionScope['SPRING_SECURITY_LAST_EXCEPTION'].message}" />
			<c:choose>
				<c:when test="${fn:contains(errorString, 'null')}">
				Caused : This domain is not accessible.
			</c:when>
				<c:otherwise>
	...
	<!-- Surendra -->
	<div class="errorblock" style="color: red">${errors}</div>
	<!-- End! -->
	<p style="margin: 0px 0px 5px 0px; text-align: left;">
	<h4>Employer Login</h4>
	</p>
	<p style="margin: 0px 0px 5px 0px; text-align: left;">
		<b>Sign In to Employer Login</b>
	</p>
	<br />
	<form name="f" action="<c:url value='/check_login' />" method='POST'>
		<p>
			<label>Username</label>&nbsp;<input type='text' name='username'><br>
			<a href="${pageContext.request.contextPath}/showForgotUserIdStep1">Forgot
				Username?</a>
		</p>
		<br />
		<p>
			<label>Password</label>&nbsp;<input type='password' name='password'><br>
			<a href="${pageContext.request.contextPath}/showForgotPasswordStep1">Forgot
				Password?</a>
		</p>
		<p>
			<input value="Sign In" type="submit" name="submit"> <input
