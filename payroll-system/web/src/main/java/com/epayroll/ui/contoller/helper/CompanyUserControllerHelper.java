 ...
 */
@Component
public class CompanyUserControllerHelper {
	public Logger logger = LoggerFactory.getLogger(CompanyUserControllerHelper.class);

	public MailMessage prepareMail(UserAddForm userAddForm) {
		System.out.println("in prepareMail");
		ResourceBundle bundle = ResourceBundle.getBundle("mail");

		String LINK = bundle.getString("LINK");

		LINK = LINK.replaceAll("LOGIN_ID", userAddForm.getUserName());
		LINK = LINK.replaceAll("PWD", RandomNumberUtils.encode(userAddForm.getPassword()));
		LINK = LINK.replaceAll("COMPANY_USER_ID", userAddForm.getUserId().toString());

		String MAIL_CONTENT = bundle.getString("MAIL.CONTENT");

		MAIL_CONTENT = MAIL_CONTENT.replace("USER_NAME", userAddForm.getUserName());
		MAIL_CONTENT = MAIL_CONTENT.replace("USER",
				userAddForm.getFirstName() + " " + userAddForm.getLastName());
		MAIL_CONTENT = MAIL_CONTENT.replace("TEMP_PASSWORD", userAddForm.getPassword());
		MAIL_CONTENT = MAIL_CONTENT.replaceAll("LINK", LINK);

		MailMessage mailMessage = new MailMessage();
		mailMessage.setMailTo(new String[] { userAddForm.getEmail() });
